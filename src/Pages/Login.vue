...
const handleLogin = async () => {
  try {
    ...
    // Save token, user and role in Pinia store
    const activeRole = res.staff.staff_roles?.find((role) => role.is_active);
    auth.login({
      token: res.access,
      user: res.staff,
        ...
        <div class="w-full max-w-md">
          ...
          <form @submit.prevent="handleLogin" class="space-y-4 md:space-y-6">
            ...
            <div class="text-right">
              <a href="#" class="text-orbit-blue text-sm underline">Forgot Password?</a>
            </div>

            <button type="submit" class="w-full block btn btn-blue-main font-bold cursor-pointer">
              Login
            </button>
          </form>
        </div>
