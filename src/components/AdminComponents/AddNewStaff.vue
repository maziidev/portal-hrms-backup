    ...
    <n-form ref="formRef" :model="form" :rules="rules">
      <div class="basic_information">
        ...
        <div class="grid md:grid-cols-2 my-4 gap-[10px]">
          ...
          <div class="mb-1">
            <n-form-item path="last_name" label="Last Name" class="block mb-2">
              <n-input
          ...
          <div class="mb-1">
            <n-form-item label="Is Academic" path="is_academic">
              <n-switch
                v-model:value="form.is_academic"
...
const formattedDate = ref(new Date(form.date_of_employment));
watch(
  () => form.date_of_employment,
  (newVal) => {
    form.date_of_employment = new Date(newVal).toISOString().slice(0, 10);
  },
);
watch(
...
const rules = {
  ...
  last_name: [
    ...
    },
    {
      validator: (rule, value) => {
        if (!/^[A-Za-z]{3,}$/.test(value)) {
          return new Error("Last name must contain only letters");
        }
        return true;
      },
      trigger: "input",
  ...
  is_academic: {
    required: true,
    type: "boolean",
    message: "Is Academic is required",
    trigger: ["change", "blur"],
  },
  date_of_birth: {
    validator(rule, value) {
